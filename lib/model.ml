...
module Board : BoardType with type t = tile list list = struct
  ...
  let valid_row (h1 : tile) (h2 : tile) (h3 : tile) (joker : int) : bool * int = 
    ...
    | Num n1, Num n2, Num n3 -> if n1.num == n2.num && n2.num == n3.num 
      then (check_same_num h1 h2 h3, joker) else 
      if n1.color == n2.color && n2.color == n3.color 
      then (check_same_color h1 h2 h3, joker) else (false, joker)
    | Num n1, Num n2, Joker | Num n1, Joker, Num n2 | Joker, Num n1, Num n2 -> 
      if n1.num == n2.num then (check_same_num h1 h2 h3, joker + 1) else 
      if n1.color == n2.color then (check_same_color h1 h2 h3, joker + 1) else 
      (false, joker + 1)
    | Num _, Joker, Joker | Joker, Joker, Num _ -> (true, joker + 2)
    | _ -> (false, joker + 3)
   
  (** given four tiles, checks if that row of tiles is valid*)
  let check_four (t1 : tile) (t2 : tile) (t3 : tile) (t4 : tile)
  (joker : int) : bool * int =
    ...
    | Num n1, Num n2, Num n3, Num n4 -> if n1.num == n2.num && n2.num == n3.num
      ...
      && n2.color != n4.color), joker) else if n2.num == n1.num + 1 
      && n3.num == n2.num + 1 && n4.num == n3.num + 1 
      then ((n1.color == n2.color && n2.color == n3.color
      && n3.color == n4.color), joker) else (false, joker)
    ...
    | Num n1, Num n2, Joker, Num n3 -> 
      if n1.num == n2.num && n2.num == n3.num then (check_same_num t1 t2 t4, joker + 1)
      else if n1.color == n2.color && n2.color == n3.color then 
      ((n2.num == n1.num + 1 && n3.num == n2.num + 2), joker + 1) else (false, joker + 1) 
    | Num n1, Joker, Num n2, Num n3 -> 
      if n1.num == n2.num && n2.num == n3.num then (check_same_num t1 t3 t4, joker + 1) 
      else if n1.color == n2.color && n2.color == n3.color then
      ((n2.num == n1.num + 2 && n3.num == n2.num + 1), joker + 1) else (false, joker + 1) 
    | Joker, Joker, Num n1, Num n2 | Joker, Num n1, Num n2, Joker | 
      Num n1, Num n2, Joker, Joker -> 
      if n1.num == n2.num then ((n1.color != n2.color), joker + 2) else 
      if n1.color == n2.color then ((n2.num == n1.num + 1), joker + 2) else 
      (false, joker + 2) 
    | Joker, Num n1, Joker, Num n2 | Num n1, Joker, Num n2, Joker -> 
      if n1.num == n2.num then ((n1.color != n2.color), joker + 2) else
      if n1.color == n2.color then ((n2.num == n1.num + 2), joker + 2) else 
      (false, joker + 2) 
    | Num n1, Joker, Joker, Num n2 -> 
      if n1.num == n2.num then ((n1.color != n2.color), joker + 2) else 
      if n1.color == n2.color then ((n2.num == n1.num + 3), joker + 2) else 
      (false, joker + 2) 
    | _ -> (false, joker)
  ...
  let rec check_row (num_joker : int) (tlst : tile list) : bool * int =
    if num_joker > 2 then (false, num_joker) else 
    match tlst with 
    | [] -> (true, num_joker)
    | _ :: [] -> (true, num_joker)
    | h1 :: h2 :: [] -> 
      (match h1, h2 with
      | Num n1, Num n2 -> if n2.num == n1.num + 1 then (true, num_joker) 
    ...
    | h1 :: h2 :: h3 :: [] -> 
      ...
      | Num n1, Joker, Num n2 -> if num_joker + 1 > 2 then (false, num_joker) 
        else if n2.num == n1.num + 2 then (true, num_joker) else (false, num_joker)
      | Num n1, Num n2, Joker | Joker, Num n1, Num n2-> 
        if num_joker + 1 > 2 then (false, num_joker) else
        if n2.num == n1.num + 1 then (true, num_joker) else (false, num_joker)
    ...
    | h1 :: h2 :: h3 :: h4 :: t -> 
      ...
      | Joker, Num n1, Num n2, _ -> if n2.num == n1.num + 1 then 
        check_row (num_joker + 1) (h3 :: h4 :: t) else (false, num_joker) 
      ...
      | Num n1, Num n2, Joker, Joker -> if n2.num == n1.num + 1 then 
        check_row (num_joker) (h3 :: h4 :: t) else (false, num_joker) 
      | Num n1, Joker, Joker, Num n3 -> if n3.num == n1.num + 3 then 
        check_row (num_joker + 2) (h4 :: t) else (false, num_joker)
      ...
      | Num n1, Num n2, Num n3, _ -> if check_same_color h1 h2 h3 then
        check_row (num_joker) (h4 :: t) else (false, num_joker)
  ...
  let rec check (board : tile list list) (jokers : int) : bool = 
    ...
    | h :: t ->  
      ...
      | _ :: [] -> false
      | _ :: _ :: [] -> false
      | h1 :: h2 :: h3 :: [] -> let checked = valid_row h1 h2 h3 jokers in 
        (match checked with 
        | (b, joker) -> if b then check t joker else false)
      | h1 :: h2 :: h3 :: h4 :: [] -> let checked = (check_four h1 h2 h3 h4 jokers) in 
        (match checked with
        | (b, joker) -> if b then check t joker else false)
      | h1 :: h2 :: h3 :: tlist -> let c = get_color h1 h2 h3 in 
        match c with
        | (b, col) -> if b then let l = h1 :: h2 :: h3 :: tlist in 
          (if check_color col l then let checked = (check_row jokers l) in 
          match checked with
          | (b, joker) -> if b then check t joker else false
